    ...
    if (isProcessEnabled()) {
        ...
        if ((mDescriptor.flags & EFFECT_FLAG_TYPE_MASK) == EFFECT_FLAG_TYPE_AUXILIARY) {
            ...
            memset(mConfig.inputCfg.buffer.raw, 0,
                   mConfig.inputCfg.buffer.frameCount*2*sizeof(int32_t));
#else
            memset(mConfig.inputCfg.buffer.raw, 0,
                   mConfig.inputCfg.buffer.frameCount*sizeof(int32_t));
#endif
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
            int32_t *in = mConfig.inputCfg.buffer.s32;
            int32_t *out = mConfig.outputCfg.buffer.s32;
            for (size_t i = 0; i < frameCnt; i++) {
                int64_t tempValue;
                tempValue = out[i] + in[i];
...
status_t AudioFlinger::EffectModule::configure()
{
    ...
    {
        mConfig.inputCfg.format = AUDIO_FORMAT_PCM_32_BIT;
        mConfig.outputCfg.format = AUDIO_FORMAT_PCM_32_BIT;
    }
#else
    mConfig.inputCfg.format = AUDIO_FORMAT_PCM_16_BIT;
    mConfig.outputCfg.format = AUDIO_FORMAT_PCM_16_BIT;
...
#ifdef MTK_HD_AUDIO_ARCHITECTURE
        // KH: TBD check above buffer size
        int32_t *buffer = new int32_t[numSamples*thread->channelCount()];
        memset(buffer, 0, numSamples * sizeof(int32_t)*thread->channelCount());
        effect->setInBuffer(buffer);
#else
