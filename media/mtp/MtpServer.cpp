...
MtpServer::MtpServer(int fd, MtpDatabase* database, bool ptp,
    ...
    :   mFD(fd),
        ...
        ,mFileTransfer(false)
        //Added Modification for ALPS00276320
		/* Added flag for ALPS01601747 */
        ,mFirstGetHandles(false)
        //revocery flag init value for ALPS01633355
...
void MtpServer::sendObjectAdded(MtpObjectHandle handle) {
	/*
	 * Added flag for ALPS01601747
	 * filter out objectAdded events until the user clicks into the storage.
	 */
	if(mFirstGetHandles){
		...
		ALOGI("Not enter storage, sendObjectAdded skip\n", handle);
}

void MtpServer::sendObjectRemoved(MtpObjectHandle handle) {
	/*
	 * Added flag for ALPS01601747
	 * filter out objectRemoved events until the user clicks into the storage.
	 */
	if(mFirstGetHandles){
		...
		ALOGI("Not enter storage, sendObjectRemoved skip\n", handle);
}
...
MtpResponseCode MtpServer::doGetObjectHandles() {
	...
	/*
	 * Added flag for ALPS01601747
	 * filter out events until the user clicks into the storage.
	 */
	mFirstGetHandles = true;
...
MtpResponseCode MtpServer::doSendObjectInfo() {
    ...
    //ALPS00973299
    ALOGI("name: %s , mSendObjectFileSize = 0x%x, maxSize: 0x%llx, \n", (const char *)name, mSendObjectFileSize, maxFileSize);
    //ALPS00973299
    ...
    if (maxFileSize != 0) {
        ...
        //ALPS00973299, if requested for support 4G-1, need to remove limitatio of 0xFFFFFFFF
        //if (mSendObjectFileSize > maxFileSize)	return MTP_RESPONSE_OBJECT_TOO_LARGE;
        //ALPS00973299
    }
...
static void deleteRecursive(const char* path) {
    ...
    while ((entry = readdir(dir))) {
        ...
        if (ret)
            ALOGE("deletePath failed for %s: %d", pathbuf, ret);
...
static void deletePath(const char* path) {
    ...
    if (stat(path, &statbuf) == 0) {
        ...
        if (ret)
            ALOGE("deletePath failed for %s: %d", path, ret);
