...
MatroskaSource::MatroskaSource(
    ...
    : mExtractor(extractor),
      ...
      mNALSizeLen(0) {
		...
		//add debug info
		ALOGD("MatroskaSource constructor,mTrackContentAddDataSize=%d",mTrackContentAddDataSize);
		for(size_t i = 0; i < mTrackContentAddDataSize; i++){
			ALOGD("mTrackContentAddData[%d] = 0x%x",i,*(mTrackContentAddData + i));
		}
		...
		ALOGD("MatroskaSource contructor mDefaultDurationNs = %lld",mDefaultDurationNs);		
#endif
...
status_t MatroskaSource::readBlock() {
	...
	ALOGV("readBlock,%s Block frameCount =%d,block timeUs =%lld",mIsAudio? "audio":"video",frameCount,timeUs);
	if (mType == MP2_3){
		...
		mPendingFrames.push_back(bigbuf);			
		 ...
		 for (uint32_t i = 0; i < frameCount; ++i) {
        	const mkvparser::Block::Frame &frame = block->GetFrame(i);
			ALOGV("readBlock,%s frame.len=%ld,mTrackContentAddDataSize=%d",mIsAudio? "audio":"video",frame.len,mTrackContentAddDataSize);
	    	...
	    	if((mDefaultDurationNs > 0) && (mType == AAC)) {
				...
				ALOGV("readBlock,%s Block frame %d timestamp %lld",mIsAudio? "audio":"video",i,timeUs + i * (mDefaultDurationNs /1000));
	    	}
	   	 	else{
...
status_t MatroskaSource::read(
    ...
    *out = NULL;
	...
	size_t size = frame->range_length();
	ALOGV("%s mType=%d,frame size =%d",mIsAudio? "audio":"video",mType,size);
...
status_t addVorbisCodecInfo(
    ...
    if (len2 > (SIZE_MAX - *codecPrivate)) {
        return ERROR_MALFORMED; // would overflow
    }
    len2 += *(codecPrivate++);
    if ((len1 + len2 + 3) > SIZE_MAX ) {
        return ERROR_MALFORMED;
    }
            ...
            case AUDIO_TRACK:
            {
                ...
                if (!strcmp("A_AAC", codecID)) {
					...
					if (atrack->GetChannels() > 2) {
						ALOGE("Unsupport MP1 Channel count=%lld", atrack->GetChannels());
						continue;
					}
					if ((atrack->GetSamplingRate() < 0.1) || (atrack->GetChannels() == 0)){
						meta->setInt32(kKeyCodecInfoIsInFirstFrame,true);
					}
				}else if ((!strcmp("A_MPEG/L2", codecID)) || (!strcmp("A_MPEG/L3", codecID))){
