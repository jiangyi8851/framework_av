...
#ifdef MTK_AOSP_ENHANCEMENT
struct OMXCodec::DeathNotifier : public IBinder::DeathRecipient {
    DeathNotifier(const sp<OMXCodec> &OMXCodec) :mOMXCodec(OMXCodec){}

    virtual void binderDied(const wp<IBinder> &) {
        ALOGE("Oops! mediaserver died!");
        mOMXCodec->setState(ERROR);
    }

protected:
    sp<OMXCodec> mOMXCodec;
...
#if 0 ////ALPS01877791 video transcoder can not destroy instances
            codec->mDeathNotifier = new DeathNotifier(codec);
            if (omx->asBinder()->linkToDeath(codec->mDeathNotifier) != OK){
                // This was a local binder, if it dies so do we, we won't care
                // about any notifications in the afterlife.
                //ALOGD("mDeathNotifier.clear() in OMXCodec::Create ");
                codec->mDeathNotifier.clear();
            }
#endif
#endif //MTK_AOSP_ENHANCEMENT
...
status_t OMXCodec::configureCodec(const sp<MetaData> &meta)
{
    ...
    else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_MS_ADPCM, mMIME) || !strcasecmp(MEDIA_MIMETYPE_AUDIO_DVI_IMA_ADPCM, mMIME))
    {
        if (!strncmp(mComponentName, "OMX.MTK.AUDIO.DECODER.ADPCM", 27))
        {
            ...
            SXLOGD("ADPCM Decode profileADPCM.pExtendData is 0x%lx", (unsigned long)profileADPCM.pExtendData);
    ...
    else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_RAW, mMIME))
    {
        ...
        if (!strcmp(mComponentName, "OMX.MTK.AUDIO.DECODER.RAW"))
        {
            ...
            SXLOGD("start configure mtk raw codec!!!");
          ...
          ? NULL : nativeWindow)
{
    ...
    {
        char name[128];
        sprintf(name, "%s DrainInputBuffer", mIsVideo ? "video" : "audio");
...
status_t OMXCodec::allocateBuffersOnPort(OMX_U32 portIndex)
{
    ...
    CODEC_LOGD("allocating %lu buffers of size %lu on %s port",
    ...
    for (OMX_U32 i = 0; i < def.nBufferCountActual; ++i)
    {
        ...
        CODEC_LOGD("allocated buffer %p on %s port", buffer,
                   portIndex == kPortIndexInput ? "input" : "output");
    }
...
status_t OMXCodec::allocateOutputBuffersFromNativeWindow()
{
    ...
    ALOGD("native_window_set_buffers_geometry W(%d), H(%d), Stride(%d), SliceH(%d), eColorFormat(0x%08X)",
          (int)def.format.video.nFrameWidth, (int)def.format.video.nFrameHeight, (int)def.format.video.nStride,
          (int)def.format.video.nSliceHeight, def.format.video.eColorFormat);
...
#ifdef MTK_AOSP_ENHANCEMENT
    //minUndequeuedBufs += 1;  // TODO: REMOVE ME, temp workaround for ALPS01309644, ALPS01436159
#ifdef MTK_SLOW_MOTION_VIDEO_SUPPORT
    if (mIsSlowMotion)
    {
...
#ifdef MTK_AOSP_ENHANCEMENT
        if ((anb != NULL) && (isDequeueFail == false))
        {
#else
        if (anb != NULL)
        {
...
#ifdef MMPROFILE_HTTP
            MMProfileLogMetaStringEx(MMP_OMXDecFBD[mIsVideo], MMProfileFlagPulse, 0, msg.u.extended_buffer_data.timestamp / 1000, "FILL_BUFFER_DONE+");
#endif //MMPROFILE_HTTP
...
#ifdef MTK_AOSP_ENHANCEMENT
                if (data1 == kPortIndexOutput && data2 != OMX_IndexVendorMtkOmxVdecGetAspectRatio && data2 != OMX_IndexVendorMtkOmxVdecGetCropInfo)
                {
...
void OMXCodec::waitClientBuffers(const Vector<BufferInfo> &buffers)
{
    ...
    while(i < buffers.size()){
        ...
        {
            if (n > WAIT_TIMES){
                ALOGE("Client did not return buffer %d for %d ms", i, WAIT_LIMIT_US/1000);
                dumpBufferOwner(buffers);
                //Intentionally trigger NE
                TRESPASS();
            }

            ALOGD("Waiting for Clirent returning buffer %d", i);
...
void OMXCodec::dumpBufferOwner(const Vector<BufferInfo> &buffers)
{
    ...
    for (size_t i = 0; i < buffers.size(); ++i)
    {
        ALOGD("buffers[i].mStatus: %d, mBuffer %p, mMediaBuffer %p",
    ...
    OMX_U32 portIndex, bool onlyThoseWeOwn)
{
    ...
    for (size_t i = buffers->size(); i-- > 0;)
    {
        ...
        CODEC_LOGD("freeing buffer %p on port %ld", info->mBuffer, portIndex);
...
bool OMXCodec::drainInputBuffer(BufferInfo *info)
{
    ...
    {
#ifdef  MMPROFILE_HTTP
        MMProfileLogMetaStringEx(MMP_OMXDecETB[mIsVideo], MMProfileFlagPulse, mPaused, 0, "drainInputBuffer when mPaused");
...
#ifdef  MMPROFILE_HTTP
    MMProfileLogMetaStringEx(MMP_OMXDecETB[mIsVideo], MMProfileFlagStart, mPaused, 0, "drainInputBuffer+");
#endif //MMPROFILE_HTTP
    ...
    CODEC_LOGD("Calling emptyBuffer on buffer %p (length %d), "
...
#ifdef  MMPROFILE_HTTP
    MMProfileLogMetaStringEx(MMP_OMXDecETB[mIsVideo], MMProfileFlagEnd, 0, timestampUs / 1000, "drainInputBuffer-");
#endif //MMPROFILE_HTTP
...
void OMXCodec::fillOutputBuffer(BufferInfo *info)
{
    ...
    if (mPortStatus[kPortIndexOutput] == SHUTTING_DOWN){
        CODEC_LOGW("mPortStatus[kPortIndexOutput] is SHUTTING_DOWN, skip fillOutputBuffer for %p",
            info->mBuffer);
        return;
    }
#endif
    ...
    CODEC_LOGD("Calling fillBuffer on buffer %p", info->mBuffer);
...
status_t OMXCodec::waitForBufferFilled_l()
{
    ...
    if (mIsEncoder)
    {
        ...
        if ((mState == LOADED)||(mState == ERROR)){
            //Once stopped, don't wait for buffer filled
            return UNKNOWN_ERROR;
        }
    ...
    MediaBuffer **buffer, const ReadOptions *options)
{
    ...
    *buffer = info->mMediaBuffer;
    ...
    if ((mFlags & kUseMaxOutputBuffers) && (mNativeWindow != NULL) && mIsVideo)
    {
        ...
        OMX_BUFFERHEADERTYPE *pBufferHeader = (OMX_BUFFERHEADERTYPE *)info->mBuffer;
