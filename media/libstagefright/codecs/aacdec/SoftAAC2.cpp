...
#ifdef MTK_AOSP_ENHANCEMENT
    while ((!inQueue.empty() || mEndOfInput || (outputDelayRingBufferSamplesAvailable() >= mStreamInfo->frameSize * mStreamInfo->numChannels &&
		outputDelayRingBufferSamplesAvailable() > 0))&& !outQueue.empty()) {
#else
    while ((!inQueue.empty() || mEndOfInput) && !outQueue.empty()) {
        ...
        if (!inQueue.empty()) {
            ...
            if ((inHeader->nFlags & OMX_BUFFERFLAG_CODECCONFIG) != 0) {
				...
				mInputBufferCountAnchor = mInputBufferCount;
#endif
            ...
            if (inHeader->nFilledLen == 0 && 
				(getBytesInDecoder() == 0 || nDecodingNotEnoughBits)) {
#else
            if (inHeader->nFilledLen == 0) {
#endif
#ifdef MTK_AOSP_ENHANCEMENT
            ...
            do {
                ...
                if (decoderErr == AAC_DEC_OK) {
...
#ifdef MTK_AOSP_ENHANCEMENT
                    ALOGD("mIsRawAac(%d), mIsErrorCsdHandled(%d), mInputBufferCount(%d) mInputBufferCountAnchor(%d)", 
                    mIsRawAac, mIsErrorCsdHandled, mInputBufferCount, mInputBufferCountAnchor);
                    if (mIsRawAac && !mIsErrorCsdHandled) {
                        ...
                        {
                            sampleIndex += 3;
                            mAACConfigSpecificData[0] = mAACConfigSpecificData[0] | ((sampleIndex & 0xE) >> 1);
                            mAACConfigSpecificData[1] = mAACConfigSpecificData[1] | ((sampleIndex & 0x1) << 7);
        ...
        while (!outQueue.empty()
            ...
            outInfo = NULL;
#ifdef MTK_AOSP_ENHANCEMENT
			ALOGV("out timestamp %lld / %d / %d", outHeader->nTimeStamp, outHeader->nFilledLen, outHeader->nFlags);
#else
