    ...
    if (mBuffer == NULL || mBuffer->size() == 0) {
        ...
        case VC1_VIDEO:
        {
            ...
            case MPEG_AUDIO:
            {
                ...
                if (startOffset < 0) {//ALPS001585646, MpegAudioHeader is lost, but we can ignore this error
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitAAC() {
    ...
    while (offset + 7 <= mBuffer->size()) {
        ...
        while (!((Compare_EQ(startCode, 0xfffu)).empty())) {
            ...
            if (startOffset < 0) {
                ALOGE("error here , no header???, lefte byte=%d",
                      mBuffer->size() - offset);
                return NULL;
            }
...
*/
        ...
        if (mFormat == NULL) {
            ...
            ALOGE("AACmAudioFrameDuration %lld sampleRate=%d",
                  mAudioFrameDuration, sampleRate);
...
*/
        ...
        if (offset + aac_frame_length > mBuffer->size()) {
            ALOGD("break aac_frame_length=%d,mBuffer->size()=%d",
    ...
    if (mBuffer->size() > 0 && frameOffsets.size() > 0
        ...
        ALOGD
            ("qian AAC correct the timestamp from %lld to %lld,mAudioFrameDuration=%lld",
             info->mTimestampUs,
             info->mTimestampUs +
             mAudioFrameDuration * frameOffsets.size(), mAudioFrameDuration);
        info->mTimestampUs =
...
bool ElementaryStreamQueue::IsIFrame(uint8_t * nalStart, size_t nalSize) {
    ...
    if ((mH264UsePPs && nalType == 7) ||    //PPS
        ...
        ALOGI("%s() nalType=%d slice_type=%d ,nalSize:%d", __FUNCTION__,
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitHEVC() {
    ...
    while ((err = getNextNALUnit(&data, &size, &nalStart, &nalSize)) == OK) {
        ...
        if (flush) {
            ...
            ALOGV
                ("[%s]flush sliceType = %u preVCLIndex:%d nals.size():%d",
...
status_t findMP3Header(const uint8_t * buf, ssize_t size,
    ...
    ALOGI
        ("%s():size:%d,Not found MP3Headr,buf:%2x %2x %2x %2x %2x %2x %2x %2x",
         __FUNCTION__, size, buf[0], buf[1], buf[2], buf[3], buf[4],
         buf[5], buf[6], buf[7]);
    ...
    if (mMP3Header == 0) {
        if (findMP3Header(data, size, &offset, &mMP3Header) != OK) {
            if (size > kMaxBytesChecked) {
                ALOGE("findMP3Header fail size>%d, skip this buffer",
                      kMaxBytesChecked);
                mBuffer->setRange(0, 0);
                return NULL;
            } else {
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitPSLPCM() {
    ...
    ALOGD
        ("PCM DEQUEUE timeUs=%d framesize is %d buffer size is %d size is %d offset is %d",
         timeUs, framesize, mBuffer->size(), size, offset);
    accessUnit->meta()->setInt64("timeUs", timeUs);
...
sp<ABuffer> ElementaryStreamQueue::dequeueAccessUnitDvbSubtitle() {
    ...
    CHECK_GE(timeUs, 0ll);
    accessUnit->meta()->setInt64("timeUs", timeUs);
    ALOGI("DVB: size:%d, timeUs:%lld", size, timeUs);
        ...
        if (mFormat != NULL && (currentStartCode == 0x00 || (sawPictureStart && currentStartCode == 0xB7))) { //ALPS00473447
            ...
            if (mSeeking) {
                if (((data[offset + 5] >> 3) & 0x7) == 1) { // I frame
                    ...
                    if (lastGOPOff != -1) {
                        tmpOff = lastGOPOff;
                        ALOGI
                            ("Send GOP when seeking, offset:%x lastGOPOff:%x",
                             offset, lastGOPOff);
                    }
            ...
            if (!sawPictureStart) {
                ...
                if(timeUs == mLastTimeUs )
                {
                    ...
                    //ALOGE("set fgInvalidTimeUs:%d",fgInvalidTimeUs);
                }else {
                    mLastTimeUs = timeUs;
                }
#endif
                accessUnit->meta()->setInt64("timeUs", timeUs);
                //ALOGE("MPEG timeUs:0x%llx",timeUs);
#ifdef MTK_AOSP_ENHANCEMENT
