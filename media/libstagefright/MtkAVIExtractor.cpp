...
#ifdef MTK_AOSP_ENHANCEMENT // Demon Deng
	LOGI("====================================="); 
...
status_t MtkAVIExtractor::parseSTRF(MtkAVIOffT pos, MtkAVIOffT end, sp<MtkAVISource> source) {
    ...
    if (source->mIsVideo) {
		...
		if (source->mIsAVC) {
			...
			ALOGD("H.264 extra data length is %d", videoExtraConfigDataSize);
			for (int s = 0; s < csd->size(); s++) {
				ALOGD("%02d: %02x", s, csd->data()[s]);
			}
			...
			ALOGD("AVCStyle = %d", source->AVCStyle);
			...
			if(source->AVCStyle == SIZE_NAL_TYPE ) {
				...
				ALOGI("AVCSizeLength = %ld",source->AVCSizeLength);
			}
...
#ifdef MTK_AUDIO_CHANGE_SUPPORT
    if (mStarted){
        LOGI("had start, forbid start again");
        return OK;
    }
#else
    CHECK(!mStarted);
...
#ifdef MTK_AUDIO_CHANGE_SUPPORT
    if (!mStarted){
        LOGI("not start, forbid stop operation");
        return OK;
    }
#else
    CHECK(mStarted);
...
status_t MtkAVISource::generateCodecData(bool full) {
    ...
    ALOGD("generate codec info , avc style is %d", SIZE_NAL_TYPE);

	if (mIsAVC && (AVCStyle == SIZE_NAL_TYPE)) {
		ALOGI("length+nalu style h.264");
        return OK;
	}
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)
        ...
        if ((strfData != NULL) && ((!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_MPEG4))
			...
			sp<ABuffer> esds = MakeESDS(strdData);
			mFormat->setData(kKeyESDS, kTypeESDS, esds->data(), esds->size());
