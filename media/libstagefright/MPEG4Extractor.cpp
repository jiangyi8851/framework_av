...
//live photo box
      ...
      if (max_size > 3840*2160*3/2)        // maybe some abnormal size
	    {
		     //mLastTrack->skipTrack = true;
          max_size = 3840*2160*3/2;
...
sp<MediaSource> MPEG4Extractor::getTrack(size_t index) {
    ...
    if (indexMfra >= 0) {
        ALOGI(" %s(),line:%d ,trackid:%d", __FUNCTION__, __LINE__,trackid);
        return new MPEG4Source(this,
                track->meta, mDataSource, track->timescale, track->sampleTable,
                mSidxEntries, mMfraEntries.editValueAt(indexMfra), trex, mMoofOffset);
    } else {
        ALOGI("%s(),line:%d ,trackid:%d,mfraEntrys.size:%x", __FUNCTION__, __LINE__,trackid, mMfraEntry.size());
        return new MPEG4Source(this,
                track->meta, mDataSource, track->timescale, track->sampleTable,
                mSidxEntries, mMfraEntry, trex, mMoofOffset);
    }
#else
    return new MPEG4Source(this,
            track->meta, mDataSource, track->timescale, track->sampleTable,
    ...
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &mode)) {
...
#ifdef MTK_AOSP_ENHANCEMENT
        ALOGI("seek time :%lld, numSidxEntries:%d", seekTimeUs, numSidxEntries);
#endif
        if (numSidxEntries != 0) {
            int64_t totalTime = 0;
        ...
        int numMfraEntries = mMovieFragments.size();
          ALOGI("%s(),line:%d, num:%x, mMovieFragments:%p", __FUNCTION__, __LINE__, numMfraEntries, &mMovieFragments);
        if (seekTimeUs < 0) {
            seekTimeUs = 0;
        }
        if (numMfraEntries > 0) {
