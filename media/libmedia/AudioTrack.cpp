...
#ifdef MTK_AOSP_ENHANCEMENT
    SXLOGD("audiotrack %p set Type %d, rate %d, fmt %d, chn %d, fcnt %u, flags %04x", 
...
if (mIsTimed ||(mFlags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) != 0)  {
    ...
    {
        ALOGD(" AudioTrack::setSampleRate return Bad Value rate = %d",rate);
        return BAD_VALUE;
    }
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT
	gAudioTrackCenter.addTrack((intptr_t)mCblk, frameCount, mSampleRate, (void*)this, afFrameCount,afSampleRate);
#endif
...
{
    ALOGD("%s userSize %d", __FUNCTION__, userSize);
