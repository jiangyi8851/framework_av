            ...
            if (what == Renderer::kWhatEOS) {
                ...
                if ((mAudioEOS || mAudioDecoder == NULL)
                 && (mVideoEOS || mVideoDecoder == NULL)) {
                    ...
                    if (err == OK && mSource->notifyCanNotConnectServerIfPossible(curPosition)) {
                        break;
                    }
#endif
                    notifyListener(MEDIA_PLAYBACK_COMPLETE, 0, 0);
                ...
                ALOGD("Tear down audio offload, fall back to s/w path if due to error.");
                int64_t positionUs;
...
void NuPlayer::reviseNotifyErrorCode(int msg,int *ext1,int *ext2) {
    if (mIsMtkPlayback && ((mDataSourceType == SOURCE_Http) && (msg == MEDIA_ERROR || msg == MEDIA_PLAY_COMPLETE ||
