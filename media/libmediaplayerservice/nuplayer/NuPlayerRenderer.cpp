...
void NuPlayer::Renderer::onPause() {
    ...
    if (mHasAudio) {
        mAudioSink->pause();
#ifdef MTK_AOSP_ENHANCEMENT
        if (offloadingAudio() && mRetryOffload) {
			notifyRetryAudioOffload();
			mRetryOffload = false;
        }
#endif
...
#ifdef MTK_AOSP_ENHANCEMENT 
        currentPositionUs = mPausedTimeUsOffload;
		mPausedTimeUsOffload = 0;
#else
        currentPositionUs = 0;
#endif
    }
...
#endif
void NuPlayer::Renderer::signalRetryOffload() {
	mRetryOffload = true;
}

void NuPlayer::Renderer::notifyRetryAudioOffload() {
    (new AMessage(kWhatRetryAudioOffload, id()))->post();
}

void NuPlayer::Renderer::onRetryAudioOffload() {
    if (mAudioOffloadTornDown) {
        return;
    }
    mAudioOffloadTornDown = true;

	ALOGD("onRetryAudioOffload");
	int64_t currentPositionUs;
    if (getCurrentPosition(&currentPositionUs) != OK) {
        currentPositionUs = 0;
    }
	mPausedTimeUsOffload = currentPositionUs;

    mCurrentOffloadInfo = AUDIO_INFO_INITIALIZER;
    sp<AMessage> notify = mNotify->dup();
    notify->setInt32("what", kWhatRetryAudioOffload);
    notify->setInt64("positionUs", currentPositionUs);
    notify->post();
}
#endif
