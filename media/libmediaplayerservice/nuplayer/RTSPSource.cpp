...
bool NuPlayer::RTSPSource::notifyCanNotConnectServerIfPossible(int64_t curPositionMs) {
	if (mQuitRightNow) {
		return false;
	}
    ...
    if (getDuration(&durationUs) == OK) {
        if (durationUs == 0 || durationUs - curPositionMs > kRTSPEarlyEndTimeUs) {
            ALOGI("RTSP timeout durationUs = %lld curPositionMs = %lld", durationUs, curPositionMs);
            sp<AMessage> notify = dupNotify();
            notify->setInt32("what", kWhatSourceError);
            notify->setInt32("err", MEDIA_ERROR_CANNOT_CONNECT_TO_SERVER);
            notify->post();
            return true;
