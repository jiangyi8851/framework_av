...
sp<MetaData> NuPlayer::GenericSource::getFormatMeta(bool audio) {
    ...
    if (err == OK && response != NULL) {
        ...
        if (mCachedSource != NULL && format != NULL) {
            ...
            if (!audio) {
                const char *mime;
                if (meta->findCString(kKeyMIMEType, &mime) && !strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_HEVC)) { 
                    meta->setInt32(kKeyMaxQueueBuffer, 4);    // due to sw hevc decode, need more input buffer
